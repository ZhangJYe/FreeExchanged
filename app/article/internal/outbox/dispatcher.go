...
func (d *Dispatcher) loadPending(ctx context.Context) ([]Event, error) {
	var pending []Event
...
ORDER BY id
LIMIT ?`, statusPending, d.batchSize)
	return pending, err
}
...
SET status = ?, last_error = '', update_time = NOW()
WHERE id = ?`, statusSent, id)
	return err
}
...
func (d *Dispatcher) markFailed(ctx context.Context, id int64, publishErr error) error {
	...
	_, err = d.conn.ExecCtx(ctx, `
UPDATE article_outbox_events
SET retry_count = retry_count + 1, last_error = ?, next_retry_at = ?, update_time = NOW()
WHERE id = ?`, message, time.Now().Add(time.Duration(delaySeconds)*time.Second), id)
	return err
}
